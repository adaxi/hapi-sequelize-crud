...
module.exports = (e) => {
  if (e.original) {
    ...
    if (e.name === 'SequelizeValidationError') { err = Boom.badData(message) } else if (e.name === 'SequelizeConnectionTimedOutError') { err = Boom.gatewayTimeout(message) } else if (e.name === 'SequelizeHostNotReachableError') { err = Boom.serverUnavailable(message) } else if (e.name === 'SequelizeUniqueConstraintError') { err = Boom.conflict(message) } else if (e.name === 'SequelizeForeignKeyConstraintError') { err = Boom.expectationFailed(message) } else if (e.name === 'SequelizeExclusionConstraintError') { err = Boom.expectationFailed(message) } else if (e.name === 'SequelizeConnectionError') { err = Boom.badGateway(message) } else err = Boom.badImplementation(message)
